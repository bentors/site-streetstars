...
export default function ProductForm({ productId = null }) {
  ...
  function handleGalleryFiles(e) {
    ...
    if (invalid) {
      ...
      showToast('error', `Arquivo inválido: ${error}`)
      e.target.value = ''
      return
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!payload.name || !payload.price || !payload.category || !payload.img) {
        showToast('error', 'Preencha nome, preço, categoria e imagem antes de salvar.')
        setLoading(false)
        return
      }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8 max-w-3xl mx-auto py-10 px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-white/10 pt-6">
          <div
            ...
            className={`border p-4 rounded cursor-pointer transition-all flex items-center justify-between group select-none ${
              formData.isActive ? 'border-green-500/50 bg-green-500/10' : 'border-white/10 bg-zinc-900'
            }`}
          >
            <div className="flex items-center gap-3">
              <div className={`w-3 h-3 rounded-full transition-all ${formData.isActive ? 'bg-green-500 shadow-[0_0_10px_#22c55e]' : 'bg-zinc-600'}`} />
              <div>
